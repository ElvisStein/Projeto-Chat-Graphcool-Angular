...
export class UserProfileComponent implements OnInit {
  ...
  isEditing = false;
  isLoading = false;
  @HostBinding('class.app-user-profile') public applyHostClass = true;
  ...
  // // Aula 288 - Ouvindo o evento "change" do input e abrindo o MatDialog
  ...
  onSave(): void {
    ...
    this.userService.updateUser(this.user)
      .pipe(take(1))
      .subscribe(
        (user: User) => message = 'Perfil atualizado!',
        error => message = this.errorService.getErrorMessage(error),
        () => {
          this.isLoading = false;
          this.showMessage(message);
